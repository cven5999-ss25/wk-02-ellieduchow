...
--\> mutate is used to add new columns to the data frame, a new column income_id was created with 4 levels– HIC, UMC, LMC, LIC
...
\<mutate is used to add new columns to the data frame, a new column income_id was created with 4 levels– HIC, UMC, LMC, LIC, each country was assigned one of the 4 levels

5.  Describe in your own words what the 'color' argument inside the `aes()` does

Color was used to sort the x,y individual data points based on income id.

6.  Describe in your own words what the `labs()` function does

It looks like the ggplot function uses the data of global waste data kg year and uses the aes() function is used to plot the data x,y and then organized by color through income ID. The labs category adds titles to the x and y value as indicated by the "".
